...
ECS::~ECS()
{
	for(Map<uint32, Array<uint8>>::iterator it = components.begin(); it != components.end(); ++it) {
		uint32 typeSize = static_cast<uint32>(BaseECSComponent::getTypeSize(it->first));
...
EntityHandle ECS::makeEntity(BaseECSComponent** entityComponents, const uint32* componentIDs, size_t numComponents)
{
	...
	newEntity->first = static_cast<uint32>(entities.size());
...
void ECS::removeEntity(EntityHandle handle)
{
	...
	uint32 srcIndex = static_cast<uint32>(entities.size() - 1);
...
void ECS::deleteComponent(uint32 componentID, uint32 index)
{
	...
	uint32 srcIndex = static_cast<uint32>(array.size() - typeSize);
...
bool ECS::removeComponentInternal(EntityHandle handle, uint32 componentID)
{
	...
	for(uint32 i = 0; i < entityComponents.size(); i++) {
		if(componentID == entityComponents[i].first) {
			...
			uint32 srcIndex = static_cast<uint32>(entityComponents.size()-1);
			uint32 destIndex = i;
...
void ECS::updateSystems(ECSSystemList& systems, float delta)
{
	...
	for(uint32 i = 0; i < systems.size(); i++) {
		...
		if(componentTypes.size() == 1) {
			uint32 typeSize = static_cast<uint32>(BaseECSComponent::getTypeSize(componentTypes[0]));
...
uint32 ECS::findLeastCommonComponent(const Array<uint32>& componentTypes, const Array<uint32>& componentFlags)
{
	...
	for(uint32 i = 0; i < componentTypes.size(); i++) {
		...
		uint32 typeSize = static_cast<uint32>(BaseECSComponent::getTypeSize(componentTypes[i]));
		uint32 size = static_cast<uint32>(components[componentTypes[i]].size()/typeSize);
		if(size <= minSize) {
			minSize = size;
			minIndex = i;
		}
		...
		Array<Array<uint8>*>& componentArrays)
{
    ...
    componentParam.resize(static_cast<uint64>(fmax(componentParam.size(), componentTypes.size())));
    componentArrays.resize(static_cast<uint64>(fmax(componentArrays.size(), componentTypes.size())));
	for(uint32 i = 0; i < componentTypes.size(); i++) {
		componentArrays[i] = &components[componentTypes[i]];
	}
	uint32 minSizeIndex = findLeastCommonComponent(componentTypes, componentFlags);

	uint32 typeSize = static_cast<uint32>(BaseECSComponent::getTypeSize(componentTypes[minSizeIndex]));
